...
Netlink::Netlink(bool linktype, int package = 0, const char* port = 0) {
	...
	if (linktype == SERVER_SIDE) {
		...
		if (verify(listensocket.socket == INVALID_SOCKET)) {
		...
		if (verify(bind(listensocket.socket, sockaddr->ai_addr, sockaddr->ai_addrlen) != 0)) {
			freeaddrinfo(sockaddr);
			compatbility_level_closesocket(listensocket.socket);
			shutdown();
		}
