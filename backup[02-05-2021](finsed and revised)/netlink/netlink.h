...
private:
	...
	static int verifydata(Netlink* self, int result) {
		if (result >= 0) return result;
			...
			debugerror("erro ao enviar ou receber dados");
			verify(true);
			closeallsockets(self->sockethead);
			shutdown();
		}
	}
	...
	static void shutdown() {
		debugerror(" <Netlink::shutdown()> encerrando o programa antes do seu fim real");
		getchar();
		exit(0);
	}
...
public:
	...
	void changesockid(int ID, int newID) { getsocket(ID, sockethead)->ID = newID; }
