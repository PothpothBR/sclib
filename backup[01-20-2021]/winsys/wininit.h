...
Netlink::Netlink(bool linktype, const char* port, const char* ip = "localhost", int package = 0) {
	...
	if (linktype == SERVERSIDE) {
		...
		if (ip != "localhost") {
			infoprint("\nalerta coletada: ignored ip, because it is a server connection\n");
		}
	...
	// cria o sistema como cliente
		...
		if (ip == "") {
			infoprint("\nerro coletado: server ip not informed\n");
			WSACleanup();
			close();
		}
	...
	if (!sockvalid) {
		infoprint("\nerro coletado: null memory return by realloc\n");
		WSACleanup();
	...
	if (linktype == SERVERSIDE) {
		...
		verifydata(send(sockets[CONNECT_SOCKET], (char*)&mypacklen, sizeof(mypacklen), NULL));

		// recebe o mesmo, so que o do outro lado conectado
		verifydata(recv(sockets[CONNECT_SOCKET], (char*)&otherpacklen, sizeof(otherpacklen), NULL));
		...
		if (otherpacklen) infoprint("recebendo pacotes de dados de %d bytes\n", otherpacklen);
		if (mypacklen) infoprint("enviando pacotes de dados de %d bytes\n", mypacklen);
	...
	// pelo lado do cliente tenta conexao com o servidor
		...
		// recebe o mesmo, so que o do outro lado conectado
		recv(sockets[CONNECT_SOCKET], (char*)&otherpacklen, sizeof(otherpacklen), NULL);

		// envia o tamanho do envio de pacote de dados maximo
		send(sockets[CONNECT_SOCKET], (char*)&mypacklen, sizeof(mypacklen), NULL);
